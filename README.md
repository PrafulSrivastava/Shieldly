...
The test suite uses an isolated `shieldher_test` database and in-process `fakeredis` — no external services required.

```bash
# Inside Docker
docker compose exec app pytest tests/ -v

# Locally (requires Python 3.11+ and a reachable Postgres)
...
| Incidents | `/api/v1/incidents` | `POST /trigger`, `POST /{id}/respond`, `POST /{id}/all-clear`, `GET /{id}` |
